...
func _init() -> void:
	...
	load("res://addons/babylonian/object-inspector/scripts/inspector_property_array.gd")
	load("res://addons/babylonian/object-inspector/scripts/inspector_property_dictionary.gd")
...
func update_inspector() -> void:
	...
	for property: Dictionary in _valid_properties:
		...
		control.size_flags_stretch_ratio = 5	
		...
		if property["usage"] == PROPERTY_USAGE_SUBGROUP:
			...
			var halo: Node = parent
			while halo is not Halo:
