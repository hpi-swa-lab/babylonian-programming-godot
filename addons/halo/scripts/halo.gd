...
@onready var print_button: TextureButton = $PrintButton
...
const TREE_LINE_PARENT_COLOR: Color = Color.MAGENTA
const TREE_LINE_CHILD_COLOR: Color = Color.GREEN
const TREE_LINE_CENTER_COLOR: Color = Color.WHITE
...
func set_target(target: CanvasItem) -> void:
	self.target = target
	self.name_tag.text = self.target.get_class()
